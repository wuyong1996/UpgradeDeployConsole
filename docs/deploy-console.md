...
周期自动发布确认提交、Git来源和部署配置均未变化时，仅更新该端最近检查时间、结果和提交，不保留本次发布记录或成功审计，检查周期保持不变。实际发布（含配置变化后的重新部署）、失败及手动操作仍保留记录；既有历史不自动清理。主机结果新增`noChanges`布尔字段，仅明确跳过发布时为true，旧适配器缺少该字段时仍保留记录；生效需同步更新API与`host.py`。
